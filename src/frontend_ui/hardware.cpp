...
void hardware_init() {
    ...
    lastPinA_State = (initialStates >> ENCODER_A_PIN) & 1;
    lastEncSwitchState = (initialStates >> ENCODER_SW_PIN) & 1;
    lastButton1State = (initialStates >> BUTTON_1_PIN) & 1;
    lastButton2State = (initialStates >> BUTTON_2_PIN) & 1;
}

void handle_hardware_inputs() {
    uint16_t pinStates = TCA.read16();
    // --- Process Encoder Rotation ---
    uint8_t pinA = (pinStates >> ENCODER_A_PIN) & 1;
    if (pinA == LOW && lastPinA_State == HIGH) {
        uint8_t pinB = (pinStates >> ENCODER_B_PIN) & 1;
    ...
    uint8_t button2State = (pinStates >> BUTTON_2_PIN) & 1;
...
void update_leds() {
    ...
    switch (currentMode) {
        ...
        case 1:
            ...
            fill_solid(leds, NUM_LEDS, CHSV(encoderValue, 255, 255));
            break;
        case 2:
            FastLED.setBrightness(200);
            FastLED.clear();
